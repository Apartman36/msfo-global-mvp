...
В учебной модели расход по отложенному налогу отражается в строке `PL_TAX` / «Расход по налогу на прибыль». Это упрощение для демонстрации связи корректировки с отчётом о прибыли и убытках.

## Cash Flow - учебное приближение

Отчёт о движении денежных средств в MVP является упрощённой образовательной аппроксимацией косвенного метода. Он помогает показать эффект трансформационных корректировок, но не является полноценной IFRS-моделью денежных потоков.
